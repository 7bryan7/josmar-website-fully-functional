...
// .wrangler/tmp/bundle-EyICRV/checked-fetch.js
...
// .wrangler/tmp/bundle-EyICRV/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-EyICRV/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-EyICRV/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
async function authMiddleware(request, env) {
  ...
  try {
    const { Database: Database2 } = await Promise.resolve().then(() => (init_db(), db_exports));
    const db = new Database2(env.DB);
    const dbUser = await db.get("SELECT id FROM users WHERE id = ? AND deleted_at IS NULL", [payload.id]);
    if (!dbUser) {
      return new Response(JSON.stringify({ error: "Unauthorized: User session is no longer active in database" }), {
        status: 401,
        headers: { "Content-Type": "application/json" }
      });
    }
  ...
  if (method === "PUT" && path === "/api/admin/settings") {
    try {
      ...
      for (const item of body) {
        ...
        statements.push(
          env.DB.prepare("UPDATE settings SET value = ?, updated_at = CURRENT_TIMESTAMP WHERE key = ?").bind(val === null || val === void 0 ? "" : String(val), item.key)
        );
      }
    ...
    try {
      ...
      const params = keys.map((k) => {
        ...
        if (k.endsWith("_id") || k.endsWith("_media_id")) {
          if (val === "" || val === "null" || val === "undefined" || val === null || val === void 0) {
            val = null;
          }
        }
        return val;
  ...
  async fetch(request, env, ctx) {
    ...
    if (method === "GET" && path.startsWith("/media/")) {
      try {
        ...
        let file = await env.MEDIA_BUCKET.get(key);
        if (!file) {
          try {
            const { Database: Database2 } = await Promise.resolve().then(() => (init_db(), db_exports));
            const db = new Database2(env.DB);
            const mediaItem = await db.get("SELECT path FROM media WHERE id = ?", [key]);
            if (mediaItem && mediaItem.path) {
              file = await env.MEDIA_BUCKET.get(mediaItem.path);
            }
          } catch (dbErr) {
